...
figure;
set(gcf,'color','w'); hold on; axis equal; axis on;
...
for k = 1:10:length(T_gc_ARKit)
    ...
    plot_camera_frame_black_only(T_gc_ARKit{k}(1:3,1:3), T_gc_ARKit{k}(1:3,4), 2.0, 'k');
end

% draw ARKit 3D trajectory
plot3(stateEsti_ARKit(1,:),stateEsti_ARKit(2,:),stateEsti_ARKit(3,:),'r','LineWidth',2.5);
plot_inertial_frame(5.0); axis equal; axis on; grid on;
