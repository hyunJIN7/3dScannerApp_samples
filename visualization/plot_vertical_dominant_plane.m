function [h_line, h_plane] = plot_vertical_dominant_plane(verticalDominantDirection, greatcircleWidth, greatcircleColor)

% generate the plane basis vectors from surface normal
tempVector = rand(3,1);
tempVector = tempVector / norm(tempVector);

pt1_basis = cross(verticalDominantDirection,tempVector);
pt1_basis = pt1_basis / norm(pt1_basis);

pt2_basis = cross(verticalDominantDirection,pt1_basis);
